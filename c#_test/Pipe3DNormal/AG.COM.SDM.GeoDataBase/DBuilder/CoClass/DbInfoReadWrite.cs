...
namespace AG.COM.SDM.GeoDataBase.DBuilder
{
    ...
    public class DbInfoReadWrite
    {
        ...
        public DbInfoReadWrite(TreeView pTreeView, PropertyGrid propertyGrid)
        {
            ...
            try
            {
                switch (pFieldType)
                {
                    ...
                    case esriFieldType.esriFieldTypeSmallInteger:
                        return Convert.ToInt16(pCellValue);
        ...
        public void WriteDBInfoToFile(string filePath)
        {
            ...
            using (FileStream fs = new FileStream(filePath, FileMode.Create))
            {
                ...
                tXmlOut.WriteComment("作者:");

                //开始
                tXmlOut.WriteStartElement("DataBaseConfig");
                //将所有的数据库写入到文件中去
                foreach (TreeNode node in m_dbTreeView.Nodes)
                {
                    //开始
                    tXmlOut.WriteStartElement("数据库");
                    //写入属性信息
                    WriteAttributeString(tXmlOut, node);
                    //递归写入子节点信息
                    WriteChildNode(tXmlOut, node);
                    //结束
        ...
        /// <summary>
        /// 功能描述:用于复制treeNode Tag
        /// 创建人:雷振京
        /// 创建时间:2021-01-28
        /// </summary>
        public void WriteDBInfoToFile(string filePath,TreeNode node)
        {
            //初始化文件流
            using (FileStream fs = new FileStream(filePath, FileMode.Create))
            {
                try
                {
                    ...
                    tXmlOut.WriteStartDocument();
                    //开始
                    tXmlOut.WriteStartElement("TreeNode");
                    tXmlOut.WriteStartElement("Node");
                    //写入属性信息
                    WriteAttributeString(tXmlOut, node);
                    //递归写入子节点信息
                    WriteChildNode(tXmlOut, node);
        ...
        /// 创建时间:2021-01-28
        /// </summary>
        public ItemProperty ReadDBInfoToTreeNode(string filePath)
        {
            try
            {
                ...
                string strPath = "TreeNode/Node";
                XmlNodeList xmlNodeList = xmlDoc.SelectNodes(strPath);
                XmlNode tXmlNode = xmlNodeList[0];
                //根据节点信息返回属性项
        ...
        private void WriteAttributeString(XmlTextWriter pXmlOut, TreeNode pTreeNode)
        {
            ...
            switch (itemProperty.DataNodeItem)
            {
                case EnumDataNodeItems.DataBaseItem:
                    ...
                    pXmlOut.WriteAttributeString("IsDefault", dbItemProperty.IsDefault.ToString());
                    break;
                #endregion
