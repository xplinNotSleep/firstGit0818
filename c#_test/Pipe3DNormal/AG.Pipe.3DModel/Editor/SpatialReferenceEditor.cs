...
namespace AG.Pipe.Analyst3DModel
{
    ...
    public class SpatialReferenceEditor : UITypeEditor
    {
            ...
            if (context != null && context.Instance != null && provider != null)
            {
                ...
                if (m_WindFormEditService != null)
                {
                    try
                    {
                        ...
                        PipeConvertManager tConvertManager = tSchemePro.GetPropertyOwner(null) as PipeConvertManager;
                        if (tConvertManager == null) return null;

                        SpatialReferenceDialog dlg = new SpatialReferenceDialog();
                        //dlg.SpatialReference = this.txtSpatialRef.Tag as ISpatialReference;
                        if (dlg.ShowDialog() == DialogResult.OK)
                        {
                            //txtSpatialRef.Tag = dlg.SpatialReference;
                            return dlg.SpatialReference;
                            //ISpatialReference tSpatialReference= dlg.SpatialReference;
                            //string sName = tSpatialReference.Name;
                            //return sName;
                        }
                    }
                    catch
