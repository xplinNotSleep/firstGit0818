...
namespace AG.Pipe.Analyst3DModel
{
    ...
    /// 提供要素类对象转换的类
    /// </summary>
    public class PipeConvertSchemeConverter : ExpandableObjectConverter 
    {
         /// <summary>
        /// 默认构造函数,初始化新实例
        /// </summary>
        public PipeConvertSchemeConverter()
        {
        }
        /// <summary>
        /// 使用参数将给定的值对象转换为指定的类型。
        ...
        {
            IWorkspace tFeatureClass = value as IWorkspace;
            if (tFeatureClass != null)
                return tFeatureClass.PathName;  //在这里我们只返回它的名称          
        ...
        {
            if (destinationType == typeof(IWorkspace))
                return true;
            else
        ...
        {
            if (sourceType == typeof(IWorkspace))
                return true;
            else
