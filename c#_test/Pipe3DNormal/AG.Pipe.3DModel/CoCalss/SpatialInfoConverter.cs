...
namespace AG.Pipe.Analyst3DModel
{
    ...
    /// 提供要素类对象转换的类
    /// </summary>
    public class SpatialInfoConverter : ExpandableObjectConverter 
    {
         /// <summary>
        /// 默认构造函数,初始化新实例
        /// </summary>
        public SpatialInfoConverter()
        {
        }
        /// <summary>
        /// 使用参数将给定的值对象转换为指定的类型。
        ...
        {
            List<PointInfo> tPointInfo = value as List<PointInfo>;
            if (tPointInfo != null)
                return CommonHelper.ConvertPointsInfo2Str(tPointInfo);  //在这里我们只返回它的名称          
