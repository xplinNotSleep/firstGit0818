    ...
    public partial class RowMergeView : DataGridView
    {
        ...
        public RowMergeView()
        {
            ...
            for (int i = 0; i < base.Rows.Count; i++)
            {
                if (i % 2 == 0)
                {
                    ...
                    base.Rows[i].DefaultCellStyle = style;
                }
            }
        ...
        private void DrawCell(DataGridViewCellPaintingEventArgs e)
        {
            ...
            if (this.MergeColumnNames.Contains(this.Columns[e.ColumnIndex].Name) && e.RowIndex != -1)
            {
                ...
                for (int i = 0; i < m_MergeRowsIndexs.Count; i++)
                {
                    int[] tInt = m_MergeRowsIndexs[i];

                    if (i % 2 == 0)
                    {
                        if (e.RowIndex >= tInt[0] && e.RowIndex < (tInt[1] + tInt[0]))
                        {
                            backBrush.Color = Color.LightBlue;
                            e.Graphics.FillRectangle(backBrush, e.CellBounds);
                    ...
                    if (tInt[1] % 2 == 1)
                    {
                        if (e.RowIndex == (tInt[0] + (int)(tInt[1] / 2)))
                        {
                            count = tInt[1];
                            DownRows = 0;
                            UpRows = (int)(tInt[1] / 2) + 1;
                    ...
                    else
                    {
                        ...
                        if (e.RowIndex == (tInt[0] + (int)(tInt[1] / 2) - 1))
                        {
                            count = tInt[1];
                            DownRows = 2;
                            UpRows = (int)(tInt[1] / 2);
            ...
            else if (this.MergeColumnNames.Contains(this.Columns[e.ColumnIndex].Name) == false && e.RowIndex != -1)
            {

            }
            if (this.Rows.Count > 1 && this.Columns.Count > 1 && e.RowIndex == this.Rows.Count - 2 && e.ColumnIndex >= 0)
            {
                //Pen gridLinePen = new Pen(gridBrush);
                //cellwidth = e.CellBounds.Width;
                //UpRows = 1;
                ...
                //backBrush.Color = Color.LightSalmon;
                ////填充颜色
                //e.Graphics.FillRectangle(backBrush, e.CellBounds);
                ////画下面的线
                //e.Graphics.DrawLine(gridLinePen, e.CellBounds.Left, e.CellBounds.Bottom - 1, e.CellBounds.Right - 1, e.CellBounds.Bottom - 1);
        ...
        /// 合并的列所要合并的行的索引
        /// </summary>
        private IList<int[]> m_MergeRowsIndexs = new List<int[]>();

        public IList<int[]> MergeRowsIndexs
        {
            get
            {
                return m_MergeRowsIndexs;
            }
            set
            {
                m_MergeRowsIndexs = value;
            }
        }
        ...
        private void RowMergeView_RowPrePaint(object sender, DataGridViewRowPrePaintEventArgs e)
        {
            //return;
            //if (e.RowIndex != -1)
            //{
            ...
            //                {
            //                    this.Rows[e.RowIndex].Cells[j].Style.BackColor = Color.White;
            //                }
            //            }
        ...
        private void RowMergeView_Paint(object sender, PaintEventArgs e)
        {
            //panel1.BringToFront();
            //panel1.Top =  200;
            //panel1.Left = 50;
